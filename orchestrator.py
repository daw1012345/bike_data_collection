...
ALL_AVAILABLE_COLLECTORS: List[CollectorDef] = [CollectorDef("polar", "polar", "The Polar H10 HR and ACC tracking module", "python3 /home/dawid/Documents/Workspace/bike_data_collection/polar_iface.py")]
        ...
        async with self._tasks_lock:
            for possible_collector in ALL_AVAILABLE_COLLECTORS:
                if possible_collector.slug in self.settings.get_collectors():
                    settings_str = self.settings.get_as_params() + f"--project={BASE_PROJECT_PATH}{project}/"
                    self._tasks.add(asyncio.create_task(process_handler(possible_collector, settings_str, self)))
...
async def process_handler(collector: CollectorDef, params: str, ctx: OrchestratorContext):
    print(f"Starting: {collector.path} {params}")

    proc = await asyncio.create_subprocess_shell(
        f"{collector.path} {params}",
...
async def start_handler(ctx, msg):
    if await ctx.is_running():
        return json.dumps({"command": "start", "result": False, "message": "Already running!"})

    if "project" not in msg or not msg["project"]:
        return json.dumps({"command": "start", "result": False, "message": "Missing project name!"})
    
    project = msg["project"]
    project_path = pathlib.Path(f"{BASE_PROJECT_PATH}{project}")

    if project_path.exists():
        return json.dumps({"command": "start", "result": False, "message": "Project already exists!"})
...
async def set_settings_handler(ctx, msg):
    if await ctx.is_running():
        return json.dumps({"command": "set_settings", "result": False, "message": "Cannot set settings while collectors are running!"})

    if "config" not in msg:
        return json.dumps({"command": "set_settings", "result": False, "message": "Corrupted message"})
...
async def get_settings_handler(ctx, msg):
    return json.dumps({"command": "get_settings", "result": True, "message": ctx.settings.get_settings()})

async def set_collectors_handler(ctx, msg):
    if await ctx.is_running():
        return json.dumps({"command": "set_collectors", "result": False, "message": "Cannot set collectors while running an experiment!"})
    
    if "collectors" not in msg:
        return json.dumps({"command": "set_collectors", "result": False, "message": "Corrupted message"})
...
async def get_collectors_handler(ctx, msg):
    return json.dumps({"command": "get_collectors", "result": True, "message": ctx.settings.get_collectors()})

async def get_state_handler(ctx, msg):
    return json.dumps({"command": "get_state", "result": True, "message": {"collectors": ctx.settings.get_collectors(), "is_running": await ctx.is_running()}})
...
async def message_handler(ctx: OrchestratorContext, websocket):
    ...
    async for message in websocket:
        ...
        try:
            ...
            await websocket.send(json.dumps({"command": "unknown", "result": False, "message": "Malformed command!"}))
            continue
        ...
        if message["command"] in HANDLERS:
            ...
            await websocket.send(json.dumps({"command": message["command"], "result": False, "message": "Malformed command!"}))
